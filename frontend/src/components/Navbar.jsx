      ...
      <div className="container px-4 mx-auto relative text-sm pb-4">
        <div className="flex justify-between items-center">
          ...
          <div className="hidden lg:flex justify-center space-x-12 items-center">
            {user?.role === 'admin'  ? (
            <div className="flex">
              <Link to="/profile" title="Profile" className="flex items-center py-2 px-3 rounded-md hover:bg-violet-700 hover:text-white mr-3">
                <User />
              </Link>
        ...
        {mobileDrawerOpen && (
          <div
            className="fixed right-0 mt-4 w-full p-12 flex flex-col justify-evenly items-center lg:hidden z-50 bg-white/90 backdrop-blur-xl shadow-2xl border border-white/20 rounded-2xl border-hidden"
          >
            <ul>
              {navItems.map((item, index) => (
            ...
            <div className="flex space-x-6 mt-8">
              {user?.role === 'admin' ? (
                <Link to="/profile" className="py-2 px-3 rounded-md hover:bg-violet-700 hover:text-white">
                  <User />
                </Link>
              ) : (
                <>
